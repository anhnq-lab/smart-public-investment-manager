    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
                    ...
                    >
                        ...
                        <div className="relative z-10">
                            ...
                            <p className="text-3xl font-black mt-2 tracking-tight drop-shadow-sm">
                                {card.value}
                                {card.suffix && <span className="text-sm font-semibold ml-1.5 text-white/60">{card.suffix}</span>}
                            </p>
                ...
                <div className="flex flex-col md:flex-row items-center gap-3">
                    ...
                    <div className="flex items-center bg-gray-100 rounded-xl p-1 gap-0.5">
                        {[
                            ...
                            >
                                {opt.label}
                                <span className={`ml-1 text-[10px] ${statusFilter === opt.value ? 'text-blue-600' : 'text-gray-400'}`}>
                                    {opt.count}
                                </span>
                            </button>
                        ))}
            ...
            <Card className="overflow-hidden border-0 shadow-sm ring-1 ring-gray-100">
                    ...
                    <table className="w-full text-left text-sm">
                        <thead>
                            <tr className="bg-gradient-to-r from-gray-50 to-gray-100/50 border-b border-gray-100">
                                <th className="px-6 py-4 text-[10px] font-extrabold uppercase tracking-[0.15em] text-gray-500">Số hợp đồng</th>
                                <th className="px-6 py-4 text-[10px] font-extrabold uppercase tracking-[0.15em] text-gray-500">Nhà thầu</th>
                                <th className="px-6 py-4 text-[10px] font-extrabold uppercase tracking-[0.15em] text-gray-500">Dự án</th>
                                <th className="px-6 py-4 text-[10px] font-extrabold uppercase tracking-[0.15em] text-gray-500 text-right">Giá trị HĐ</th>
                                <th className="px-6 py-4 text-[10px] font-extrabold uppercase tracking-[0.15em] text-gray-500 text-center">Giải ngân</th>
                                <th className="px-6 py-4 text-[10px] font-extrabold uppercase tracking-[0.15em] text-gray-500 text-center">Ngày ký</th>
                                <th className="px-6 py-4 text-[10px] font-extrabold uppercase tracking-[0.15em] text-gray-500 text-center">Trạng thái</th>
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <span className="text-[10px] text-gray-400">Gói {contract.PackageID?.slice(-5) || '—'}</span>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2.5">
                                                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center ring-1 ring-slate-200">
                                                    <Building2 className="w-3.5 h-3.5 text-slate-500" />
                                                </div>
                                                <span className="font-medium text-gray-800 max-w-[200px] truncate text-[13px]" title={contractorName}>
                                                    {contractorName}
                                                </span>
                                            </div>
                                        </td>
                                        ...
                                        {/* Project */}
                                        <td className="px-6 py-4">
                                            <span className="text-gray-500 text-xs max-w-[200px] truncate block leading-relaxed" title={projectName}>
                                                {projectName}
                                            </span>
                                        </td>
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <span className="font-bold text-gray-900 font-mono text-sm tracking-tight">{formatCurrency(contract.Value)}</span>
                                        </td>

                                        {/* Payment Progress */}
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col items-center gap-1.5">
                                                <div className="w-full max-w-[120px] bg-gray-100 rounded-full h-2 overflow-hidden">
                                                    <div
                                                        className={`h-full rounded-full transition-all duration-1000 ${payProgress.percent >= 80 ? 'bg-gradient-to-r from-emerald-400 to-emerald-500' :
                                                            payProgress.percent >= 40 ? 'bg-gradient-to-r from-blue-400 to-blue-500' :
                                                                payProgress.percent > 0 ? 'bg-gradient-to-r from-amber-400 to-amber-500' : 'bg-gray-200'}`}
                                                        style={{ width: `${Math.min(payProgress.percent, 100)}%` }}
                                                    ></div>
                                                </div>
                                                <div className="flex items-center gap-1.5 text-[10px]">
                                                    <span className="font-bold text-gray-600">{payProgress.percent.toFixed(0)}%</span>
                                                    <span className="text-gray-300">·</span>
                                                    <span className="text-gray-400">{payProgress.count} đợt</span>
                                        ...
                                        <td className="px-6 py-4 text-center">
                                            {contract.Status === ContractStatus.Executing ? (
                                                <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase bg-blue-50 text-blue-700 ring-1 ring-blue-100">
                                                ...
                                                <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase bg-yellow-50 text-yellow-700 ring-1 ring-yellow-100">
                ...
                {/* Summary Footer */}
                <div className="bg-gradient-to-r from-gray-50 to-blue-50/30 border-t border-gray-100 px-6 py-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-6">
                            <div className="flex items-center gap-2">
                                <div className="w-2 h-2 rounded-full bg-blue-500 animate-pulse"></div>
                                <span className="text-xs text-gray-500">Đang thực hiện: <span className="font-bold text-gray-700">{stats.executingCount}</span></span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="w-2 h-2 rounded-full bg-emerald-500"></div>
                                <span className="text-xs text-gray-500">Đã thanh lý: <span className="font-bold text-gray-700">{stats.liquidatedCount}</span></span>
                            </div>
                            <div className="w-px h-4 bg-gray-200"></div>
                            <span className="text-xs text-gray-500">Tổng giá trị: <span className="font-bold text-gray-900">{formatCurrency(stats.totalValue)}</span></span>
                        </div>
                        <span className="text-xs text-gray-400">{filteredContracts.length} hợp đồng</span>
                    </div>
                </div>
                ...
                {filteredContracts.length === 0 && (
                    <div className="p-20 text-center">
                        ...
                        <p className="text-gray-600 font-bold text-lg">Không tìm thấy hợp đồng</p>
