...
export const ProjectDocumentsTab: React.FC<ProjectDocumentsTabProps> = ({
    ...
    const renderFolderTree = (parentId: string | undefined, level = 0) => {
        ...
        return (
            <div className={`space-y-0.5 ${level > 0 ? 'ml-4 border-l border-gray-200/50 pl-2' : ''}`}>
                {children.map(folder => {
                    ...
                    return (
                        <div key={folder.FolderID}>
                            ...
                            >
                                <FolderIcon className={`w-4 h-4 shrink-0 ${isActive ? 'text-blue-600 fill-blue-100' : 'text-gray-400 group-hover:text-gray-500'}`} />
                                <span className="truncate flex-1">{folder.Name}</span>
                                {docCount > 0 && (
                                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${isActive ? 'bg-blue-200 text-blue-800' : 'bg-gray-100 text-gray-500'}`}>
                                        {docCount}
                                    </span>
                                )}
                            </div>
                                ...
                                {isExpanded && (
                                    <div className="px-5 py-4 border-t border-gray-100 space-y-2">
                                        ...
                                        {category.stage === ProjectStage.Preparation && investmentPolicy && (
                                            <div className="bg-blue-50/80 rounded-lg p-4 border border-blue-100 mb-3">
                                                <div className="flex items-start gap-3">
                                                    ...
                                                    <span className="px-2 py-0.5 bg-blue-200 text-blue-800 text-[10px] font-bold rounded-full uppercase shrink-0">Đã có</span>
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        {category.stage === ProjectStage.Preparation && feasibilityStudy && (
                                            <div className="bg-emerald-50/80 rounded-lg p-4 border border-emerald-100 mb-3">
                                                <div className="flex items-start gap-3">
                                                    ...
                                                    <span className="px-2 py-0.5 bg-emerald-200 text-emerald-800 text-[10px] font-bold rounded-full uppercase shrink-0">Đã có</span>
                                                </div>
                                            </div>
                                        )}
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <div className="flex items-center gap-2 shrink-0">
                                                        {hasDoc ? (
                                                            <>
                                                                <span className="text-xs px-2.5 py-1 bg-emerald-50 text-emerald-700 rounded-lg font-bold flex items-center gap-1">
