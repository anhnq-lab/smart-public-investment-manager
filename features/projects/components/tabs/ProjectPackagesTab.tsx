...
export const ProjectPackagesTab: React.FC<ProjectPackagesTabProps> = ({ projectID }) => {
    ...
    const getStatusColor = (status: PackageStatus) => {
        switch (status) {
            ...
            case PackageStatus.Awarded: return 'bg-green-100 text-green-700'; // Assuming Contracted mapped to Awarded or needs fix
    ...
    const getStatusLabel = (status: PackageStatus) => {
        switch (status) {
            ...
            case PackageStatus.Awarded: return 'Đã trúng thầu';
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        <thead className="bg-gray-50 text-gray-600 font-medium border-b border-gray-100">
                            <tr>
                                <th className="px-6 py-3">Số hiệu & Tên gói thầu</th>
                                <th className="px-6 py-3">Giá gói thầu</th>
                                <th className="px-6 py-3">Hình thức</th>
                                <th className="px-6 py-3">Trạng thái</th>
                                <th className="px-6 py-3">Nhà thầu trúng thầu</th>
                                ...
                                <tr key={pkg.PackageID} className="hover:bg-gray-50 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-gray-600 line-clamp-1" title={pkg.PackageName}>{pkg.PackageName}</span>
                                            <span className="text-xs text-gray-400 mt-1">{pkg.Field === 'Construction' ? 'Xây lắp' : 'Tư vấn'}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 text-gray-600">
                                        <div className="flex flex-col gap-1">
                                            <span>{pkg.SelectionMethod === 'OpenBidding' ? 'Đấu thầu rộng rãi' : 'Chỉ định thầu'}</span>
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${getStatusColor(pkg.Status)}`}>
                                    ...
                                    <td className="px-6 py-4">
                                        {pkg.WinningContractorID ? (
                                            <div className="flex flex-col">
                                                <span className="text-gray-800 font-medium">Công ty CP Xây dựng Hà Tĩnh</span>
                                                <span className="text-xs text-green-600">Giá: {formatCurrency(pkg.WinningPrice || 0)}</span>
                                            </div>
                                        ) : (
