        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 flex flex-col justify-between">
                    ...
                    <div>
                        <p className="text-2xl font-black text-gray-800 dark:text-slate-100">{formatFullCurrency(stats.totalPlanned)}</p>
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 flex flex-col justify-between">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                            <div className="h-1.5 w-24 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden">
                                <div className="h-full bg-emerald-500" style={{ width: `${stats.rate}%` }}></div>
                            </div>
                            <span className="text-xs font-bold text-gray-600 dark:text-slate-300">{stats.rate.toFixed(1)}%</span>
                        </div>
                    </div>
                </div>
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 flex flex-col justify-between">
                    ...
                    <div>
                        {alerts.length > 0 ? (
                            ...
                            <p className="text-sm font-bold text-gray-400 dark:text-slate-500">Không có cảnh báo rủi ro.</p>
                        )}
                    </div>
                </div>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-gray-50 dark:divide-slate-700">
                            {plans.map((plan) => (
                                <tr key={plan.PlanID} className="hover:bg-blue-50/30 dark:hover:bg-blue-900/20 transition-colors">
                                    <td className="px-6 py-4">
                                        <p className="font-black text-gray-800 dark:text-slate-100">{plan.Year}</p>
                                        <p className="text-[10px] text-gray-400 dark:text-slate-500 font-mono">{plan.PlanID}</p>
                                    </td>
                                    <td className="px-6 py-4 text-gray-600 dark:text-slate-300 font-medium">
                                        {plan.DecisionNumber}
                                        <div className="text-[10px] text-gray-400 dark:text-slate-500 italic">Ngày: {plan.DateAssigned}</div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 text-xs font-bold text-gray-500 dark:text-slate-400">{plan.Source}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200 dark:border-slate-700 flex justify-between items-center bg-gray-50/50 dark:bg-slate-800/50">
                    ...
                    <div className="flex gap-2">
                        <button className="px-3 py-1.5 bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-600 text-gray-600 dark:text-slate-300 text-[10px] font-bold rounded-lg flex items-center gap-2 transition-all">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-gray-50 dark:divide-slate-700">
                            {disbursements.map((d) => (
                                <tr key={d.DisbursementID} className="hover:bg-emerald-50/30 dark:hover:bg-emerald-900/20 transition-colors">
                                    <td className="px-6 py-4 font-mono font-bold text-gray-800 dark:text-slate-100">{d.TreasuryCode}</td>
                                    <td className="px-6 py-4 text-gray-600 dark:text-slate-300">{d.Date}</td>
                                    <td className="px-6 py-4">
                                        <p className="font-semibold text-gray-800 dark:text-slate-100">{d.CapitalPlanID}</p>
                                        <p className="text-[10px] text-gray-400 dark:text-slate-500 italic">Thanh toán đợt...</p>
                                    </td>
                                    <td className="px-6 py-4 text-center">
                                        <span className="px-2 py-1 bg-gray-100 dark:bg-slate-700 border border-gray-200 dark:border-slate-600 text-gray-600 dark:text-slate-300 rounded text-[10px] font-mono font-bold">{d.FormType}</span>
                                    </td>
